...
-->
<!--
  One dashboard widget. Twelve kinds share this file so a dashboard is a
  list of {type, params} and nothing else. Live where cheap (my issues),
  one-shot elsewhere; `tick` re-runs the one-shot loads (wallboard refresh).
-->
<script lang="ts">
  ...
  import { IssuePriority, type Decision, type Issue, type IssueStatus, type Project, type Sprint, type TimeSpendReport } from '@hcengineering/tracker'
  ...
  async function load (): Promise<void> {
    ...
    try {
      switch (type) {
        case 'mine': {
          const list = await client.findAll(tracker.class.Issue, { assignee: me, status: { $in: openIds } }, { limit: 300 })
        ...
        case 'due': {
          const list = await client.findAll(tracker.class.Issue, { ...(params.everyone === true ? {} : { assignee: me }), status: { $in: openIds }, dueDate: { $lt: Date.now() + (params.days ?? 7) * DAY } }, { limit: 50, sort: { dueDate: SortingOrder.Ascending } })
          rows = list.filter((i) => i.dueDate != null).slice(0, params.limit ?? 8)
          await resolveNames(rows.map((i) => i.assignee))
          break
        }
        case 'stale': {
          rows = await client.findAll(tracker.class.Issue, { status: { $in: openIds }, modifiedOn: { $lt: Date.now() - (params.days ?? 7) * DAY } }, { limit: params.limit ?? 8, sort: { modifiedOn: SortingOrder.Ascending } })
          break
        }
        case 'sprints': {
          const sprints: Sprint[] = await client.findAll(tracker.class.Sprint, { state: 'active' })
        ...
        case 'workload': {
          const list = await client.findAll(tracker.class.Issue, { status: { $in: openIds }, assignee: { $ne: null } }, { limit: 3000 })
        ...
        case 'decisions': {
          decisions = await client.findAll(tracker.class.Decision, {}, { limit: params.limit ?? 6, sort: { modifiedOn: SortingOrder.Descending } })
          break
        }
        case 'query': {
        ...
        case 'pie': {
          ...
          const q: Record<string, any> = params.project !== undefined ? { space: params.project } : {}
          const list = await client.findAll(tracker.class.Issue, { ...q, ...(params.includeDone === true ? {} : { status: { $in: openIds } }) }, { limit: 5000 })
          await resolveNames(list.map((i) => i.assignee))
          const key = (i: Issue): string => (field === 'status' ? statusName.get(i.status) ?? '—' : field === 'priority' ? prioLabel[i.priority] : field === 'project' ? projectName.get(i.space) ?? '—' : i.assignee != null ? names.get(i.assignee) ?? '—' : 'Unassigned')
        ...
        case 'cvr': {
          ...
          const q: Record<string, any> = params.project !== undefined ? { space: params.project } : {}
          const created = await client.findAll(tracker.class.Issue, { ...q, createdOn: { $gte: since } }, { limit: 5000 })
          ...
          for (const m of msgs) {
            ...
            if (u?.attrKey === 'status' && done.has(u.set[0] as Ref<IssueStatus>) && !resolvedAt.has(m.objectId)) resolvedAt.set(m.objectId, m.createdOn ?? m.modifiedOn)
          }
          const dayList: number[] = []
          for (let d = since; d <= today.getTime(); d += DAY) dayList.push(d)
          const a = dayList.map(() => 0)
          const b = dayList.map(() => 0)
          const idx = (t: number): number => Math.floor((t - since) / DAY)
          for (const i of created) if (i.createdOn !== undefined) a[Math.min(dayList.length - 1, idx(i.createdOn))]++
          for (const t of resolvedAt.values()) b[Math.min(dayList.length - 1, Math.max(0, idx(t)))]++
          series = { days: dayList, a, b }
          break
        }
        case 'sla': {
          rows = await client.findAll(tracker.class.Issue, { status: { $in: openIds }, slaDue: { $lt: Date.now() + (params.hours ?? 24) * 3_600_000 } }, { limit: params.limit ?? 10, sort: { slaDue: SortingOrder.Ascending } })
          await resolveNames(rows.map((i) => i.assignee))
          break
        }
        case 'activity': {
  ...
  const TITLES: Record<string, string> = {
    mine: 'Assigned to me', due: 'Due soon', stale: 'Gone quiet', sprints: 'Active sprints', workload: 'Workload', decisions: 'Decisions',
    query: 'Query', pie: 'Breakdown', cvr: 'Created vs resolved', sla: 'SLA at risk', activity: 'Recent activity', hours: 'Hours this week'
  }
  export const title = (): string => (params.title as string | undefined) ?? TITLES[type] ?? type
...
<div class="w" class:w--wall={wall}>
  {#if busy && rows.length === 0 && groups.length === 0}
    <p class="muted">…</p>
  {/if}
  {#if errors.length > 0}
  ...
  {#if type === 'mine' || type === 'workload' || type === 'hours'}
    <div class="bars">
      {#each groups as g (g.label)}
        <div class="bar-row"><span class="bar-row__label" class:bar-row__label--wide={type !== 'mine'}>{g.label}</span><span class="track"><span class="fill" class:fill--blue={type === 'workload'} style="width: {(g.n / maxN) * 100}%" /></span><span class="bar-row__n">{g.n}{type === 'hours' ? 'h' : ''}</span></div>
      {/each}
      {#if groups.length === 0 && !busy}<p class="muted">Nothing here.</p>{/if}
    </div>
  {/if}

  {#if ['mine', 'due', 'stale', 'query', 'sla'].includes(type)}
  ...
  {#if type === 'sprints'}
    {#each progress as p (p.label)}
      <div class="sprint">
        <div class="sprint__head"><span class="sprint__name">{p.label}</span><span class="sprint__meta">{p.sub}</span></div>
        <span class="track track--wide"><span class="fill" style="width: {p.total === 0 ? 0 : (p.done / p.total) * 100}%" /></span>
  ...
  {#if type === 'decisions'}
    {#each decisions as d, idx (d._id)}
      <button class="row motion-rise" style="--i: {idx}" on:click={() => { showPanel(view.component.EditDoc, d._id, d._class, 'content') }}>
        <span class="row__title">{d.title}</span><span class="row__meta">{ago(d.modifiedOn)}</span>
      </button>
    {/each}
  ...
  {#if type === 'pie'}
    <div class="pie">
      ...
      <ul class="pie__legend">
        {#each groups.slice(0, 8) as g}<li><i class="sw" style="background: {g.color}" />{g.label}<span class="muted">{g.n}</span></li>{/each}
      </ul>
    </div>
  {/if}
  ...
  {#if type === 'cvr' && series !== undefined}
    ...
    <svg viewBox="0 0 300 100" class="line-chart" role="img">
      <path d={series.a.map((v, i) => `${i === 0 ? 'M' : 'L'}${lx(i, n).toFixed(1)},${ly(v, max).toFixed(1)}`).join(' ')} class="ln ln--red" />
      <path d={series.b.map((v, i) => `${i === 0 ? 'M' : 'L'}${lx(i, n).toFixed(1)},${ly(v, max).toFixed(1)}`).join(' ')} class="ln ln--lime" />
    </svg>
    <div class="legend"><span><i class="sw sw--red" />created {series.a.reduce((a, b) => a + b, 0)}</span><span><i class="sw sw--lime" />resolved {series.b.reduce((a, b) => a + b, 0)}</span></div>
  {/if}

  {#if type === 'activity'}
    {#each feed as f, idx (f.when + f.text)}
      <button class="row motion-rise" style="--i: {idx}" on:click={() => { showPanel(view.component.EditDoc, f.id, tracker.class.Issue, 'content') }}>
        <span class="row__title">{f.text}</span><span class="row__meta">{ago(f.when)}</span>
      </button>
    {/each}
  {/if}
</div>
...
<style lang="scss">
  ...
  .ln { fill: none; stroke-width: 2; stroke-linejoin: round; &--red { stroke: #e0475b; } &--lime { stroke: var(--accent-brand); } }
